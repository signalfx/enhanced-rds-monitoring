...
from lambda_script import e, pull_metric_names, parse_logs
...
class TestLambdaComponents(unittest.TestCase):
    ...
    def check_common_elements_standard(self, entry):
        ...
        self.assertEqual(dimensions['instanceResourceID'], 'db-H4UK4SA7E62QPMAYTMPK5XWETQ', 'Incorrect instance resource id')
        self.assertEqual(dimensions['AWSUniqueId'], 'rds_ppr9b9oge80i99_us-east-1_1', 'Incorrect AWSUniqueId dimension')
        self.assertEqual(dimensions['Namespace'], 'AWS/RDS', 'Incorrect namespace dimension')
        self.assertEqual(dimensions['EngineName'], 'MYSQL', 'Incorrect db engine name')
        self.assertIsInstance(entry['timestamp'], int, 'Timestamp not an integer')

    def check_common_elements_sql_server(self, entry):
        dimensions = entry['dimensions']
        self.assertEqual(dimensions['instanceResourceID'], 'db-YWCA2G6UQEA3NYZ54IS6XEBGUE', 'Incorrect instance resource id')
        self.assertEqual(dimensions['AWSUniqueId'], 'rds_trevor_us-west-2_1', 'Incorrect AWSUniqueId dimension')
        self.assertEqual(dimensions['Namespace'], 'AWS/RDS', 'Incorrect namespace dimension')
        self.assertEqual(dimensions['EngineName'], 'SqlServer', 'Incorrect db engine name')
        self.assertIsInstance(entry['timestamp'], int, 'Timestamp not an integer')

    def check_common_elements_aurora(self, entry):
        dimensions = entry['dimensions']
        self.assertEqual(dimensions['instanceResourceID'], 'db-F3AZGY5JTAS65TT4E7U3CDS6AM', 'Incorrect instance resource id')
        self.assertEqual(dimensions['AWSUniqueId'], 'rds_trevor_eu-west-1_1234', 'Incorrect AWSUniqueId dimension')
        self.assertEqual(dimensions['Namespace'], 'AWS/RDS', 'Incorrect namespace dimension')
        self.assertEqual(dimensions['EngineName'], 'Aurora', 'Incorrect db engine name')
        self.assertIsInstance(entry['timestamp'], int, 'Timestamp not an integer')
    ...
    def test_standard_parsing(self):
        ...
        metric_entries = parse_logs(
            '1',
            'arn:aws:lambda:us-east-1:946288580872:function:testEnhancedRDSLambda',
            my_sql_dict,
        ...
        metrics_to_check = self.grab_metrics(metric_entries, self.STANDARD_METRICS_CHECK)
        ...
        self.assertEqual(load_avg['value'], 0.29, 'Incorrect loadAverageMinute.one')

        memory_cached = metrics_to_check['memory.cached']
        self.check_common_elements_standard(memory_cached)
        self.assertEqual(memory_cached['value'], 3288068, 'Incorrect memory.cached')
        ...
        self.assertEqual(network_tx['dimensions']['interface'], 'eth0', 'Incorrectly parsed network metrics')
        ...
        self.assertEqual(diskio_await['dimensions']['device'], 'rdsdev', 'Incorrectly parsed diskIO metrics')

        filesys_total = metrics_to_check['fileSys.total']
        self.check_common_elements_standard(filesys_total)
        self.assertEqual(filesys_total['value'], 103053476, 'Incorrect fileSys.total')
        self.assertEqual(filesys_total['dimensions']['name'], 'rdsfilesys', 'Incorrectly parsed filesys metrics')
        self.assertEqual(filesys_total['dimensions']['mountPoint'], '/rdsdbdata', 'Incorrectly parsed filesys metrics')

        os_processes_cpu = metrics_to_check['OSprocesses.cpuUsedPc']
        self.check_common_elements_standard(os_processes_cpu)
        self.assertEqual(os_processes_cpu['value'], 0.5, 'Incorrectly parsed process metrics')
    ...
    def test_sql_server_parsing(self):
        ...
        # Grab the metrics we want
        metrics_to_check = self.grab_metrics(metric_entries, self.SQL_SERVER_METRICS_CHECK)
        ...
        self.assertEqual(memory_kern_paged_kb['value'], 136120, 'Incorrect memory.kernPagedKb')

        system_handles = metrics_to_check['system.handles']
        self.check_common_elements_sql_server(system_handles)
        self.assertEqual(system_handles['value'], 14874, 'Incorrect system.handles')

        disks_avail_pc = metrics_to_check['disks.availPc']
        self.check_common_elements_sql_server(disks_avail_pc)
        self.assertEqual(disks_avail_pc['value'], 99.22, 'Incorrect disks.availPc')
        self.assertEqual(disks_avail_pc['dimensions']['name'], 'rdsdbdata', 'Incorrectly parsed disks metrics')

        network_rd_bytes_ps = metrics_to_check['network.rdBytesPS']
        self.check_common_elements_sql_server(network_rd_bytes_ps)
        self.assertEqual(network_rd_bytes_ps['value'], 0, 'Incorrect network.rdBytesPS')
        self.assertEqual(network_rd_bytes_ps['dimensions']['interface'], 'Ethernet 2', 'Incorrectly parsed network metrics')

        rds_processes_working_set_priv_kb = metrics_to_check['RDSprocesses.workingSetPrivKb']
        self.check_common_elements_sql_server(rds_processes_working_set_priv_kb)
        self.assertEqual(rds_processes_working_set_priv_kb['value'], 215996, 'Incorrect RDSprocesses.workingSetPrivKb')
    ...
    def test_aurora_parsing(self):
        ...
        # Grab the metrics we want
        metrics_to_check = self.grab_metrics(metric_entries, self.AURORA_METRICS_CHECK)
        ...
        load_average_minute_fifteen = metrics_to_check['loadAverageMinute.fifteen']
        self.check_common_elements_aurora(load_average_minute_fifteen)
        self.assertEqual(load_average_minute_fifteen['value'], 0.13, 'Incorrect loadAverageMinute.fifteen')
        ...
        self.assertEqual(tasks_sleeping['value'], 233, 'Incorrect tasks.sleeping')
        ...
        self.assertEqual(network_rx['value'], 58724202.3, 'Incorrect network.rx')
        self.assertEqual(network_rx['dimensions']['interface'], 'eth0', 'Incorrectly parsed network metrics')

        diskio_write_throughput = metrics_to_check['diskIO.writeThroughput']
        self.check_common_elements_aurora(diskio_write_throughput)
        self.assertEqual(diskio_write_throughput['value'], 476409.8, 'Incorrect diskIO.writeThroughput')
        self.assertEqual(len(diskio_write_throughput['dimensions'].keys()), 4, 'Incorrectly parsed diskIO metrics')

        filesys_max_files = metrics_to_check['fileSys.maxFiles']
        self.check_common_elements_aurora(filesys_max_files)
        self.assertEqual(filesys_max_files['value'], 2097152, 'Incorrect fileSys.maxFiles')
        self.assertEqual(filesys_max_files['dimensions']['name'], 'rdsfilesys', 'Incorrectly parsed fileSys metrics')
        self.assertEqual(filesys_max_files['dimensions']['mountPoint'], '/rdsdbdata', 'Incorrectly parsed fileSys metrics')

        os_processes_vss = metrics_to_check['OSprocesses.vss']
        self.check_common_elements_aurora(os_processes_vss)
        self.assertEqual(os_processes_vss['value'], 691648, 'Incorrect OSprocesses.vss')
